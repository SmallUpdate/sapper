...
const
  ...
  fieldWidth = 20;
  fieldHeight = 10; 
  countOfMines = fieldWidth * fieldHeight div 10; // 100% : 10 = 10%
  cellSize = 60;
...
begin
  ...
  CenterWindow;
  SetWindowTitle('Sapper - by Pesniakov Igor');
...
begin
  ...
  begin
    ...
    begin
      ...
      if (field[x][y] = 9) or (openedCells = fieldWidth * fieldHeight - countOfMines) then
        isGameOver := true;
    end
    else
      for var dy := -1 to 1 do
...
begin
  ...
  begin
    ...
    begin
      if (y > 0) and not isOpened[x][y] and not firstTurn then
      begin
        if isFlag[x][y] then
          FillR(x, y, RGB(162, 209, 73), RGB(170, 215, 81))
        else
